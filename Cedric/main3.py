...
class GPCR_homology_modeling:
    ...
    def blank_model_modelisation(self):
        ...
        self.blank_model=my_model("blank.pdb", self.seq_model, self.seq_template, self.sequence_name, None)
        
        ## blank_model + teminus_folding
        self.actions.append(["terminus_folding"])
        self.actions.append(["modelisation","blank_model"])
        self.set_actions()
        self.run()
        self.actions=[]
        ...
        shutil.move("blank_model/"+self.sequence_name+".B99990001.pdb", "blank.pdb")
        shutil.rmtree("blank_model")
        os.mkdir("blank_model")
        shutil.copy("blank.pdb", "blank_model/blank.pdb")  
        ...
        for action in self.actions:
            ...
            if action[0] in ["rotation", "translation"]:                
                ...
                self.list_modelisations.append(modelisation(job_name=action[1],
                                                         ...
                                                         terminus_folding=terminus_folding))
            ...
            elif action[0]=="terminus_folding":
                terminus_folding=True
    ...
    def helix_hbond_optimisation(self):
        ...
        outset=200
        self.matrix_ref=[[outset for i in xrange(len(self.list_acceptor))] for j in xrange(len(self.list_donor))]

        with open("scores.csv", "w") as fileout:
            pass

        ### INITIALISATION ###
        for step in xrange(1,201,1):       
            self.actions=[]
            for i in xrange(self.n_proc-2):## for each model
                ## apply 5 change in model
                for change in xrange(3): 
                    self.actions.append(["init_template"])
                    helix=rd.choice([TM for TM in self.TMs])
                    pivot = int(self.TMs[helix][0])
                    start = pivot-int(self.TMs[helix][1])
                ...
                for indice_donor, line in enumerate(self.matrix_ref):
                    ##[2,5,1]==>[0,0,1,1,1,1,1,2]
                    value_weight=[]
                    for value,n in enumerate(line):
                        value_weight+=[value for tmp in xrange(n)]
                    ##random choice
                    for indice_acceptor in rd.sample(value_weight,3):
                        self.actions.append(['restraint', "upperdistance", self.list_donor[indice_donor], self.list_acceptor[indice_acceptor], 6, 3])
                        pass
            ...
            ##self.run(start_analysis=start, end_analysis=end)
            self.run()
            self.matrix_ref=self.get_hbond_bis(self.matrix_ref)
         
         
            
        ## save hbond matrix
        with open("result_hbond.csv", "w") as fileout:
            for line in self.matrix_ref:
                fileout.write("\t".join([str(i) for i in line])+"\n")
        ...
        for step in xrange(1,201,1):  
            ...
            for i in xrange(self.n_proc-2):## for each model
              ...
              ## add Hbond restraints
                for indice_donor, line in enumerate(self.matrix_ref):
                    ...
                    value_weight=[]
                    for value,n in enumerate(line):
                        value_weight+=[value for z in xrange(n)]
                    ##random choice
                    for indice_acceptor in rd.sample(value_weight,3):
                        ##self.actions.append(['restraint', "distance", self.list_donor[indice_donor], self.list_acceptor[indice_acceptor], 3, max(2-0.001*line[indice_acceptor], 1.2, 100)])
                        self.actions.append(['restraint', "upperdistance", self.list_donor[indice_donor], self.list_acceptor[indice_acceptor], 4, 2])
            ...
            name=self.get_results(step)
                
            with open("scores.csv", "a") as fileout:
                fileout.write(str(self.score_best)+"\n")
        ...
        print self.actions_best
        self.actions=[]
        for action in self.actions_best:
            try:
                if action[1]!="upperdistance":
                    self.actions.append(action)
            except:
                pass
        ## self.actions=[action for action in self.actions_best if action[1]!="upperdistance"]## All actions but not for Hbond
        ##We add really HBond
        for (acceptor, donor) in self.hbond_list_best:
            
            donor_name=donor.get_full_id()[4][0]+":"+str(donor.get_full_id()[3][1])+":"+donor.get_full_id()[2]
            acceptor_name=acceptor.get_full_id()[4][0]+":"+str(acceptor.get_full_id()[3][1])+":"+acceptor.get_full_id()[2]
            self.actions.append(['restraint', "upperdistance", donor_name, acceptor_name, 4, 3])
        
        ## modelisation
        self.actions.append(["modelisation","final_model"])
    ...
    def get_results(self, step):
        ...
        for res in result_list: ## each modelisation
            hbond_list=res["hbond_bis"]
            score=0
            for (acceptor, donor) in hbond_list:## each Hbond
                if not (donor.name=="N" or acceptor.name=="O"):
                    ...
                    score+=self.matrix_ref[indice_donor][indice_acceptor]

                    
                if score>self.score_best:
                    name=res["filename"]
                    self.score_best=score
                    self.hbond_list_best=res["hbond_bis"]
                    self.actions_best=res["actions"]
    ...
    def get_hbond_bis(self, matrix_ref):
        for modelisation in self.list_modelisations:
            hbond_list=modelisation.get_hbond_bis()
            ##matrix_ref=[[1 for i in xrange(len(self.list_acceptor))] for j in xrange(len(self.list_donor))]
