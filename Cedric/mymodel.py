...
class MyModel(automodel):
    ...
    def special_restraints(self, aln):
        ...
        #CG# --> exemples : ['distance', 'OH:110', 'N:123', 10, 0.01] or ['patch_DISU', '110', '123']
        rsr=self.restraints
        at=self.atoms
        restraints=self.parent.restraints

        #CG# rsr.add(secondary_structure.alpha(self.residue_range('151:', '156:')))
        ...
        if self.terminus_folding:
            ...
            #KC#CG# largeur=min([self.parent.blank_model.x_max,-self.parent.blank_model.x_min,self.parent.blank_model.y_max,-self.parent.blank_model.y_min])
            ...
            for atom in self.atoms:
                ...
                if atom.residue.index in self.parent.blank_model.C_ter:
                    ...
                    #CG# rsr.add(forms.upper_bound(group=physical.xy_distance,feature=features.distance(atom,down_center),mean=largeur, stdev=1))
                                                
                for i,idx in enumerate(self.parent.blank_model.N_ter):
                    if idx==atom.residue.index:
                        #CG# rsr.add(forms.upper_bound(group=physical.xy_distance,feature=features.distance(atom,center),mean=self.parent.blank_model.z_max*1.1, stdev=1))
