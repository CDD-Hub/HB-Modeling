    ...
    def is_stacking(self, other):
        ...
        for name_atom1 in parametres.dico_cycles[self.residue.resname]["cycle"]:
            for name_atom2 in parametres.dico_cycles[other.residue.resname]["cycle"]:
                d=distance(self.residue[name_atom1].get_coord(), other.residue[name_atom2].get_coord())
                if d<dist:
                    dist=d
...
class plan:
    ...
    def distance(self, point_M):
        ...
        xm,ym,zm=point_M
        d=abs(self.a*xm+self.b*ym+self.c*zm)/np.sqrt(self.a*self.a+self.b*self.b+self.c*self.c)
